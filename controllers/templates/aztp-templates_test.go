...
func TestAztpRenderYamlTemplates(t *testing.T) {
	var data AztpTemplateData
	data.AztpImage = im
...
func TestRenderAztpTemplates(t *testing.T) {
	var data AztpTemplateData
	data.AztpImage = "my image"
	objects, err := RenderAztpService(data, "full")
	if err != nil {
		t.Error("failed to render aztp templates", err)
	}
	if len(objects) != len(fullVariantemplates) {
		l := log.New(os.Stderr, "", 0)
